function EditarEntradaVehiculo(){
 ...
 if(fechaEntrada.value == ""){
  ...
  /*alert("Datos enviados"+ idnumPatrimonio + numUltimaPrueba  + estadoAlcoholimetro +
    ...
    + '&observEntrada=' + observEntrada.value + '&NumeroPlaca=' + NumeroPlaca.value;
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '<h4>Actualizacion Completada !</h4>';
         result += '</div>';
        __('_AJAX_EditarEntradaVehiculo_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_EditarEntradaVehiculo_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=EditarEntradaVehiculo',true); //Abre una conexión con ajax
...
window.onload = function(){
    var btnEditarEntradaVehiculo=__('btnEditarEntradaVehiculo');
   btnEditarEntradaVehiculo.addEventListener("click",EditarEntradaVehiculo);
