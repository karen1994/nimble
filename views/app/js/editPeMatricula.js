function editarPersonaMatricula() {
...
// Parametros capturados del formulario
  ...
  connect.onreadystatechange = function() {
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '</div>';
        __('_AJAX_Persona_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_Persona_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=editPeMatricula',true); //Abre una conexión con ajax
...
window.onload= function(){
  ...
  var btnGuardarPersona= __('btnGuardarPersona');
  var btnEditarPersona= __('btnEditarPersona');

btnGuardarPersona.addEventListener("click", editarPersonaMatricula);
btnEditarPersona.addEventListener("click", mostrar);
