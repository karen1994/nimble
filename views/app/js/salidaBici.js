function salidaBici() {
  ...
  connect.onreadystatechange = function() {
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '</div>';
        __('_AJAX_SalBici_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_SalBici_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=salidaBici',true); //Abre una conexión con ajax
...
function regresar3(){
  $("#secSalidaBici").show();
  $("#secPersonaSalidaBici").hide();
}



window.onload= function(){
  var btnGuardarSalBici = __('btnGuardarSalBici');
  var Terminar1= __('Terminar1');
  var Terminar2 = __('Terminar2');
  var regresar= __('regresar');

  btnGuardarSalBici.addEventListener("click", salidaBici);
