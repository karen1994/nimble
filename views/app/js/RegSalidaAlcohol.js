function regSalidaAlcoholimetro(){
  var connect, form, response, result, FechaSalidaAlcohol, HoraSalidaAlcohol, OfiEntregaSalida,
   OfiRecibeSalida, ObservacionesSalida, NumeroPatrimonio,tipoActivo;
 ...
 if(FechaSalidaAlcohol.value == ""){
  ...
  //************** Se abre la conexion, si todo esta bien
        form = 'FechaSalidaAlcohol=' + FechaSalidaAlcohol.value + '&HoraSalidaAlcohol=' + HoraSalidaAlcohol.value  +
    '&OfiEntregaSalida=' + OfiEntregaSalida.value + '&OfiRecibeSalida=' + OfiRecibeSalida.value + '&ObservacionesSalida=' +
    ObservacionesSalida.value + '&NumeroPatrimonio=' + NumeroPatrimonio.value + '&tipoActivo=' + tipoActivo.value;
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '<h4>Registro Completado !</h4>';
         result += '</div>';
        __('_AJAX_SalidaAlcoholimetro_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_SalidaAlcoholimetro_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=regSalidaAlcoholimetro',true); //Abre una conexión con ajax
...
window.onload = function(){
    var btnSalidaAlcohol=__('btnSalidaAlcohol');
  btnSalidaAlcohol.addEventListener("click",regSalidaAlcoholimetro);
