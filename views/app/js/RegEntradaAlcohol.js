function regEntradaAlcoholimetro(){
 ...
 if(FechaEntradaAlcohol.value == ""){
  ...
  /*alert("Datos enviados"+ idnumPatrimonio + numUltimaPrueba  + estadoAlcoholimetro +
      ...
      form = 'FechaEntradaAlcohol=' + FechaEntradaAlcohol.value + '&HoraEntradaAlcohol=' + HoraEntradaAlcohol.value +
      '&UltimaPrueba=' + UltimaPrueba.value + '&OfiEntregaEntrada=' + OfiEntregaEntrada.value + '&OfiRecibeEntrada=' +
      OfiRecibeEntrada.value + '&observacionesEntrada='+ observacionesEntrada.value + '&NumeroPatrimonio=' + NumeroPatrimonio.value
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '<h4>Registro Completado !</h4>';
         result += '</div>';
        __('_AJAX_RegEntradaAlcoholimetro_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_RegEntradaAlcoholimetro_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=RegEntradaAlcoholimetro',true); //Abre una conexión con ajax
...
window.onload = function(){
    var btnRegEntradaAlcoholimetro=__('btnRegEntradaAlcoholimetro');
    btnRegEntradaAlcoholimetro.addEventListener("click",regEntradaAlcoholimetro);
