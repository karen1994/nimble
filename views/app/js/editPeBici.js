...
// Parametros capturados del formulario
  ...
  connect.onreadystatechange = function() {
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '</div>';
        __('_AJAX_Persona_Bici_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_Persona_Bici_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=editPeBici',true); //Abre una conexión con ajax
...
btnGuardarPersona.addEventListener("click", editarPersonaBici);
