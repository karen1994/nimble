function editarMotivoMatricula() {
...
// Parametros capturados del formulario
  ...
  //************** Se abre la conexion, si todo esta bien
      ...
      if(numArticulo.value == ""){
              alert("El campo número de arículo es requerido");
              numArticulo.focus();
              $("#artMo").addClass('has-error');
           }else if (desMotivo.value == ""){
             alert("El campo motivo de la detención es requerido");
            desMotivo.focus();
              $("#descripMo").addClass('has-error');
       }
      else{
  ...
  connect.onreadystatechange = function() {
    ...
    if(connect.readyState == 4 && connect.status == 200) { //Revisa el estado de la conexión con ajax
      if(connect.responseText == 1) {
        ...
        result += '</div>';
        __('_AJAX_Motivo_').innerHTML = result;
        location.reload();
      } else {
        __('_AJAX_Motivo_').innerHTML = connect.responseText; //Se muestran los errores
      }
  ...
  connect.open('POST','ajax.php?mode=editMoMatricula',true); //Abre una conexión con ajax
...
function datosCorrectos(){
     if (numArticulo.value != ""){
           $("#artMo").removeClass('has-error');
            $("#artMo").addClass('has-success');
     }
     if (desMotivo.value != ""){
             $("#descripMo").removeClass('has-error');
              $("#descripMo").addClass('has-success');
     }
...
window.onload= function(){
  ...
  btnGuardarMotivo.addEventListener("click", editarMotivoMatricula);
